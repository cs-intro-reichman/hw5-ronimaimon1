...
public class Scrabble {
	...
	public static String createHand() {
        ...
        int indexA = rand.nextInt(HAND_SIZE - 2);
        int indexE = rand.nextInt(HAND_SIZE - 2);
        while (indexE == indexA) {
            indexE = rand.nextInt(HAND_SIZE - 2);
        }

        hand.insert(indexA, 'a');
        hand.insert(indexE, 'e');
    ...
    // the stream of characters coming from the keyboard. Used for reading the user's inputs.
    	...
    	while (hand.length() > 0) {
        	...
        	if (input.equals(".")) {
            	// If user wants to finish the hand, break out of the loop
            	break;
        	}
        
        	// Check if the input word is valid (exists in dictionary) and is a valid subset of the hand
        	if (isWordInDictionary(input) && MyString.subsetOf(input, hand)) {
            	...
            	// Remove the letters of the input word from the hand
            	for (char c : input.toCharArray()) {
                	hand = hand.replaceFirst(String.valueOf(c), "");
           		}
        	} 
			else {
    	...
    	// After the hand is over, show the total score
    	if (hand.length() == 0) {
	...
	public static void playGame() {
		...
		while (true) {
			...
			if (input.equals("e")) {
				// If user chooses to end the game, exit the loop
				break;
			} 
